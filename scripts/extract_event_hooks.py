...
def get_event_hooks(domain_url, api_token, limit=200):
    headers = {
        "Authorization": f"SSWS {api_token}",
        "Accept": "application/json",
    }

    logger.info("Fetching event hooks.")
    base = _ensure_domain_str(domain_url).rstrip("/")
    url = f"{base}/api/v1/eventHooks?limit={limit}"

    hooks = []
    while url:
        resp = requests.get(url, headers=headers)
        if resp.status_code != 200:
            logger.error("Error fetching event hooks: %s %s", resp.status_code, resp.text)
            break

        try:
            data = resp.json()
        except ValueError:
            logger.error("Invalid JSON received for event hooks")
            break

        if isinstance(data, list):
            hooks.extend(data)
        else:
            logger.error("Unexpected response format for event hooks: %s", type(data))
            break
