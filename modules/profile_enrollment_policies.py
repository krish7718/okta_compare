...
def compare_profile_enrollment_policies(envA_domain, envA_token, envB_domain, envB_token, limit=200):
    """
    Compare profile enrollment policies between Env A and Env B.
    ...
    for name, polA in dictA.items():
        if name not in dictB:
            diffs.append({
                ...
                "Recommended Action": f"Create profile enrollment policy '{name}' in Env B",
                "Priority": "🔴 Critical"
            })
            continue

        polB = dictB[name]
        rulesA = get_profile_enrollment_policy_rules(baseA, envA_token, polA.get("id"))
        rulesB = get_profile_enrollment_policy_rules(baseB, envB_token, polB.get("id"))
        ...
        if _rules_signature(rulesA) != _rules_signature(rulesB):
            diffs.append({
                ...
                "Recommended Action": f"Align profile enrollment rules for '{name}'",
                "Priority": "🟠 Medium"
            })
        else:
            matches.append({
    ...
    for name in dictB:
        if name not in dictA:
            diffs.append({
                ...
                "Recommended Action": f"Review extra profile enrollment policy '{name}' in Env B",
                "Priority": "🟡 Low"
            })
